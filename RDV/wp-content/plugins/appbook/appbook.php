...
if ( !class_exists( 'AppBook' ) ) {
	class AppBook 
	{
		...
		public function appInstall() {
			...
			// add role
			add_role( 'user-app-book', __('Utilisateur de réservation', $this->slug), array( 'read' => false, 'level_0' => true ) );
			global $wp_roles;
		...
		public function assets()
		{
			...
			// wp_register_script( $this->slug.'_moment', plugins_url().'/'.$this->slug.'/assets/js/moment.js', array(), false, false );
		...
		public function createMenu()
		{
			...
			/*$menu_events = __('Evènementiels', $this->slug);
			$menu_events_exists = wp_get_nav_menu_object( $menu_events );

			if( !$menu_events_exists){
			    $menu_events_id = wp_create_nav_menu($menu_events);
			    wp_update_nav_menu_item($menu_events_id, 0, array(
			        'menu-item-title' =>  __('Offres/Evènements', $this->slug),
			        'menu-item-status' => 'publish'));
			}*/
			...
			if( !$menu_managing_exists){
			    ...
			    wp_update_nav_menu_item($menu_managing_id, 0, array(
			        'menu-item-title' =>  __("Configuration de l'application", $this->slug),
			        'menu-item-status' => 'publish'));
			    ...
			    /*wp_update_nav_menu_item($menu_managing_id, 0, array(
			        'menu-item-title' =>  __("Durée d'ocupation des tables", $this->slug),
			        'menu-item-status' => 'publish'));*/
		    }

		    $menu_booking = __('Module de réservation', $this->slug);
