...
class AppStats
{
	...
	private static $default_colors = array(
		array('width' => 400, 'height' => 400, 'fill' => false, 'borderColor' => 'rgba(54, 190, 112, 0.2)'),
		array('width' => 400, 'height' => 400, 'fill' => false, 'borderColor' => 'rgba(112, 76, 161, 0.2)'),
		array('width' => 400, 'height' => 400, 'fill' => false, 'borderColor' => 'rgba(72, 183, 203, 0.2)'),
		array('width' => 400, 'height' => 400, 'fill' => false, 'borderColor' => 'rgba(238, 197, 106, 0.2)'),
		array('width' => 400, 'height' => 400, 'fill' => false, 'borderColor' => 'rgba(223, 97, 114, 0.2)')
	);
	...
	public function getData($service_id = null, $from = null, $to = null ) 
	{
		...
		if ($service_id === null)
		{
			...
			foreach ($services as $service) {
				$employees = $this->employee->getByPoste($service->service_id);
				if($employees)
				{
					foreach ($employees as $employee) {
						$ps = $this->period->getPeriodByEmployee($employee->employee_id);
						$p = array();
						foreach ($ps as $value) {
							$p[] = $value['period_id'];
						}
		...
		foreach ($dates as $date) 
        {
            ...
            foreach ($countByDates as $_service_id => $val) {
            	if ( $value !== null && isset($val[$date['date']]) )
            	{
